...
impl ProtocolV2 {
    ...
    pub fn run(&self) -> Vec<TrialResult> {
        ...
        for (t_idx, target) in self.targets.iter().enumerate() {
            for (d_idx, &diff) in self.difficulties.iter().enumerate() {
                ...
                println!("Scenario: Target={} difficulty={} ({} scenarios total)", t_idx, diff_label, self.targets.len() * self.difficulties.len());
                ...
                for &algo in &algorithms {
                    for trial_id in 0..self.trials_per_scenario {
                        ...
                        let result = self.run_trial(algo, target, &seed_strand, run_seed, t_idx, diff_label, trial_id);
    ...
    fn run_trial(&self, algo: &str, target: &str, seed: &Strand, run_seed: u64, t_idx: usize, diff: &str, trial_id: usize) -> TrialResult {
        ...
        let best_specimen = match algo {
            "GA" | "GA-NoXover" => match sandbox.evolve(fitness_fn, &[seed.clone()], run_seed) {
                Ok(s) => s,
                Err(_) => Specimen { strand: seed.clone(), fitness: 0.0, output: vec![], cycles: 0, halt_reason: crate::vm::HaltReason::Error },
            },
            "MC" => {
                let mut best_f = 0.0;
                ...
                for _ in 0..self.budget {
                    ...
                    for j in 0..strand.bases.len() {
                        strand.bases[j] = [crate::types::Nucleotide::A, crate::types::Nucleotide::T, crate::types::Nucleotide::G, crate::types::Nucleotide::C][rng.next_usize(4)];
                    }
                    let f = fitness_fn(&Specimen { strand: strand.clone(), fitness: 0.0, output: vec![], cycles: 0, halt_reason: crate::vm::HaltReason::Normal });
                    if f > best_f {
                        best_f = f;
                        best_s = strand;
                    }
                }
                Specimen { strand: best_s, fitness: best_f, output: vec![], cycles: 0, halt_reason: crate::vm::HaltReason::Normal }
            },
            "SRW" => sandbox.random_walk(fitness_fn, seed, self.budget, run_seed),
            "HC" => {
                // Hill-climb: accept ONLY improvements
                let mut rng = Rng::new(run_seed);
                let mut best = Specimen { strand: seed.clone(), fitness: fitness_fn(&Specimen { strand: seed.clone(), fitness: 0.0, output: vec![], cycles: 0, halt_reason: crate::vm::HaltReason::Normal }), output: vec![], cycles: 0, halt_reason: crate::vm::HaltReason::Normal };
                for _ in 0..self.budget {
                    ...
                    candidate_strand.bases[idx] = [crate::types::Nucleotide::A, crate::types::Nucleotide::T, crate::types::Nucleotide::G, crate::types::Nucleotide::C][rng.next_usize(4)];
                    
                    let f = fitness_fn(&Specimen { strand: candidate_strand.clone(), fitness: 0.0, output: vec![], cycles: 0, halt_reason: crate::vm::HaltReason::Normal });
                    if f > best.fitness {
                        best.fitness = f;
        ...
        let printable_ascii_rate = if let Ok(s) = decoded {
            let printable = s.chars().filter(|&c| (c as u32) >= 32 && (c as u32) <= 126).count();
...
pub fn generate_summary(results: &[TrialResult]) -> String {
    ...
    for r in results {
        algo_stats.entry(r.algorithm.clone()).or_default().push(r.best_fitness);
        algo_integrity.entry(r.algorithm.clone()).or_default().push(r.printable_ascii_rate);
    }
    ...
    for algo in &algos {
        ...
        summary.push_str(&format!("| {} | {:.4} | {:.4} | {:.1}% | {:.4} |\n", algo, mean, median, int_rate * 100.0, stddev));
    }

    summary.push_str("\n## Relative Improvement (vs HC Baseline)\n\n");
    let hc_mean = algo_stats.get("HC").map(|s| s.iter().sum::<f64>() / s.len() as f64).unwrap_or(0.0);
    
    for algo in &algos {
        if *algo == "HC" || *algo == "MC" { continue; }
        let mean = algo_stats[*algo].iter().sum::<f64>() / algo_stats[*algo].len() as f64;
        let improvement = if hc_mean > 0.0 { (mean / hc_mean - 1.0) * 100.0 } else { 0.0 };
        summary.push_str(&format!("- **{}**: {:.2}% improvement over HC\n", algo, improvement));
    }
