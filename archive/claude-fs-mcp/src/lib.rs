...
fn get_backup_dir() -> PathBuf {
    get_claude_root().join("backup/sessions")
}
...
pub fn create_backup() -> Result<String, FsError> {
    ...
    // Exclude the backup directory itself to prevent recursion bombs.
    for entry in WalkDir::new(&root)
        .follow_links(true)
        .into_iter()
        .filter_entry(|e| e.path() != backup_dir) // EXCLUDE BACKUP DIR
