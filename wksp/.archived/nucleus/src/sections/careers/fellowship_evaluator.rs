//! Fellowship readiness evaluator — assess qualification for PV fellowship programs
...
pub fn FellowshipEvaluatorPage() -> impl IntoView {
    let ratings: Vec<(RwSignal<u8>, &'static str, &'static str)> = vec![
        (RwSignal::new(0), "FE01", "Years of Experience — depth of hands-on PV work (1=<2yr, 5=10+yr)"),
        (RwSignal::new(0), "FE02", "Publications — peer-reviewed safety/PV publications record"),
        (RwSignal::new(0), "FE03", "Presentations — conference talks, poster presentations, invited lectures"),
        (RwSignal::new(0), "FE04", "Mentoring — track record of developing junior PV professionals"),
        (RwSignal::new(0), "FE05", "Regulatory Interactions — direct engagement with health authorities"),
        (RwSignal::new(0), "FE06", "Specialized Knowledge — expertise in niche PV areas (signal, RMP, REMS)"),
        (RwSignal::new(0), "FE07", "Professional Development — continuing education, certifications, courses"),
        (RwSignal::new(0), "FE08", "Peer Recognition — awards, committee memberships, editorial roles"),
    ];
    ...
    let category = Signal::derive(move || {
        ...
        else if avg < 2.0 { "Early Career" }
        else if avg < 3.0 { "Building Profile" }
        else if avg < 4.0 { "Competitive Candidate" }
        else if avg < 4.5 { "Strong Candidate" }
        else { "Fellowship Ready" }
    });
    ...
    view! {
        <div class="mx-auto max-w-4xl px-4 py-8">
            <h1 class="text-3xl font-bold text-white font-mono uppercase tracking-tight">"Fellowship Readiness Evaluator"</h1>
            <p class="mt-2 text-slate-400">"Evaluate your qualifications for PV fellowship and advanced recognition programs (1-5)."</p>
