...
//!
//! ```rust
//! use nexcore_vigilance::skills::{lookup_compliance_level, lookup_smst_component};
//!
...
/// Compliance level metadata for skill validation.
///
/// Defines the 5-tier Diamond compliance model:
/// - Bronze: Basic SKILL.md with YAML frontmatter
/// - Silver: + scripts/ directory
/// - Gold: + references/ and templates/
...
/// Compile-time compliance level lookup table (O(1) access)
...
///
/// # Arguments
/// * `name` - Level name (bronze, silver, gold, platinum, diamond)
///
/// # Returns
/// Static reference to the compliance level, or None if not found.
pub fn lookup_compliance_level(name: &str) -> Option<&'static ComplianceLevel> {
    COMPLIANCE_LEVELS.get(&name.to_lowercase())
}

/// Get all compliance levels ordered by tier (Bronze → Diamond).
...
/// SMST (Skill Machine Specification Type) component metadata.
///
/// The 8 components that define a machine-executable skill specification:
/// - INPUTS: Input parameters and types
/// - OUTPUTS: Return values and types
/// - STATE: Internal state variables
/// - OPERATOR_MODE: Behavioral modes (STANDARD/CAUTIOUS/AGGRESSIVE)
/// - PERFORMANCE: Complexity bounds (TIME/SPACE/MEMORY)
/// - INVARIANTS: Conditions that must always hold
/// - FAILURE_MODES: Error conditions and responses
...
pub struct SmstComponent {
    ...
    pub required: bool,
    /// Weight in SMST scoring (0.0 - 1.0, sums to 1.0)
    pub weight: f64,
    /// Human-readable description
    pub description: &'static str,
...
/// Compile-time SMST component lookup table (O(1) access)
...
/// Lookup SMST component by name.
///
/// Handles normalization:
/// - Case-insensitive ("inputs" → "INPUTS")
/// - Spaces/dashes to underscores ("operator mode" → "OPERATOR_MODE")
///
/// # Arguments
/// * `name` - Component name in any format
///
/// # Returns
/// Static reference to the component, or None if not found.
...
/// Skill category metadata for classification.
///
/// Categories help with:
/// - Routing decisions (compute-intensive → Rust delegation)
...
/// Compile-time skill category lookup table (O(1) access)
...
///
/// Handles normalization:
/// - Case-insensitive ("ALGORITHMS" → "algorithms")
/// - Underscores to hyphens ("text_processing" → "text-processing")
...
///
/// Used by the execution engine for DAG-based skill execution:
/// - root: Entry point
/// - decision: Conditional branch
/// - action: Execute command (terminal)
/// - delegate: Call another skill (terminal)
/// - error: Handle failure (terminal)
#[derive(Debug, Clone, Copy, Serialize)]
pub struct NodeType {
    /// Node type name
    pub name: &'static str,
...
/// Compile-time decision tree node type lookup (O(1) access)
...
/// Lookup decision tree node type by name.
...
///
/// Provides a unified interface for querying any taxonomy type.
...
///
/// # Arguments
/// * `query_type` - One of: "compliance", "smst", "category", "node"
/// * `key` - The lookup key
///
/// # Example
/// ```rust
/// use nexcore_vigilance::skills::taxonomy::query_taxonomy;
///
/// let result = query_taxonomy("compliance", "diamond");
/// assert!(result.found);
/// ```
        ...
        "compliance" | "level" => {
            if let Some(level) = lookup_compliance_level(key) {
                TaxonomyQueryResult {
                    ...
                    key: key.to_string(),
                    found: true,
                    data: Some(serde_json::to_value(level).unwrap_or_default()),
                }
            } else {
                TaxonomyQueryResult {
                    query_type: "compliance".to_string(),
        ...
        "category" => {
            if let Some(category) = lookup_skill_category(key) {
                TaxonomyQueryResult {
                    ...
                    key: key.to_string(),
                    found: true,
                    data: Some(serde_json::to_value(category).unwrap_or_default()),
                }
            } else {
                TaxonomyQueryResult {
                    query_type: "category".to_string(),
...
/// Result of listing all entries in a taxonomy.
#[derive(Debug, Serialize, Deserialize)]
...
/// # Example
/// ```rust
/// use nexcore_vigilance::skills::taxonomy::list_taxonomy;
///
/// let result = list_taxonomy("compliance");
/// assert_eq!(result.count, 5); // Bronze, Silver, Gold, Platinum, Diamond
/// ```
