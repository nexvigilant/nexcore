...
//!
//! Unified parsing, validation, and schema extraction for YAML and TOML files.
//!
//! ## Key Use Cases
//!
...
pub struct ParseResult {
    ...
    pub status: String,
    /// Format detected ("yaml" or "toml")
    pub format: String,
    /// Parsed data as JSON value
    pub data: JsonValue,
...
/// Taxonomy schema information extracted from YAML
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct TaxonomySchema {
    ...
    /// Root key of the taxonomy
    pub root_key: String,
    /// Total number of nodes
    pub node_count: usize,
    /// Maximum depth
    pub max_depth: usize,
    /// Whether it contains conditions
    pub has_conditions: bool,
    /// Whether it contains actions
...
pub struct DecisionTreeNode {
    ...
    pub id: String,
    /// Condition expression (if branch node)
    pub condition: Option<String>,
    /// Action to execute (if leaf or action node)
...
/// Analysis result for a decision tree
#[derive(Debug, Serialize, Deserialize, Clone)]
...
/// # Errors
///
/// Returns `ConfigError::TomlParse` if parsing fails.
...
/// # Errors
///
/// Returns `ConfigError` if parsing fails for both formats.
...
///
/// Extracts all nodes, conditions, actions, and computes metrics.
///
/// # Errors
///
/// Returns `ConfigError::YamlParse` if parsing fails.
pub fn analyze_decision_tree(content: &str) -> Result<DecisionTreeAnalysis, ConfigError> {
    ...
    let branch_nodes = nodes.len().saturating_sub(leaf_nodes);
...
/// # Errors
///
/// Returns `ConfigError::YamlParse` if no valid frontmatter found or parsing fails.
pub fn parse_yaml_frontmatter(content: &str) -> Result<JsonValue, ConfigError> {
    let re = FRONTMATTER_RE
        .as_ref()
        .map_err(|e| ConfigError::YamlParse(format!("frontmatter regex: {e}")))?;
    ...
    // Try fallback for content that might not have a newline after closing ---
...
fn extract_top_level_keys(value: &JsonValue) -> Vec<String> {
    match value {
...
fn calculate_depth(value: &JsonValue) -> usize {
    match value {
...
fn detect_schema_type(data: &JsonValue) -> Option<String> {
    ...
    if keys.iter().any(|k| {
        *k == "tree" || *k == "decisions" || *k == "conditions" || *k == "nodes" || *k == "root"
    }) {
    ...
    if keys
        ...
        && keys
            .iter()
            .any(|k| *k == "triggers" || *k == "description" || *k == "intent")
    {
...
"#;
    ...
    #[test]
    fn test_taxonomy_validation_deep() {
        let yaml = "categories:\n  - cat1\nname: deep-taxonomy";
        let result = validate_schema(yaml, Some("taxonomy"));
        assert!(result.valid);
    }
    ...
    fn test_count_nodes() {
        let yaml = "a:\n  b: 1\n  c: 2\nd: 3";
        let result = parse_yaml(yaml).unwrap();
        let count = count_nodes(&result.data);
        assert!(count >= 4); // At least a, b, c, d
    }

    #[test]
    fn test_decision_tree_complex() {
        let yaml = r#"
name: complex-tree
tree:
  condition: "x > 0"
  children:
    - condition: "x > 10"
      children:
        - action: "very_large"
        - action: "medium"
    - action: "small_or_negative"
"#;
        let analysis = analyze_decision_tree(yaml).unwrap();
        assert_eq!(analysis.name, "complex-tree");
        assert!(analysis.max_depth >= 2);
        assert!(analysis.leaf_nodes >= 3);
    }
}
