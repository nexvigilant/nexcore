...
use leptos::prelude::*;
use crate::api::metrics::{StatusResponse, MetricsResponse, Alert};
...
pub async fn get_status() -> Result<StatusResponse, ServerFnError> {
    ...
    client.status().await.map_err(|e| ServerFnError::new(e.to_string()))
}
...
pub async fn get_metrics() -> Result<MetricsResponse, ServerFnError> {
    ...
    client.get_metrics().await.map_err(|e| ServerFnError::new(e.to_string()))
}
...
pub async fn get_alerts() -> Result<Vec<Alert>, ServerFnError> {
    ...
    client.get_alerts().await.map_err(|e| ServerFnError::new(e.to_string()))
}

/// Dashboard home page
#[component]
pub fn HomePage() -> impl IntoView {
    let status = Resource::new(|| (), |_| get_status());
    let metrics = Resource::new(|| (), |_| get_metrics());
                                ...
                                <div class="space-y-2">
                                    {m.metrics.iter().map(|(k, v)| view! {
                                        <div class="flex justify-between py-1 border-b border-gray-700">
                                            ...
                                            <span class="text-green-400 font-mono">{format!("{:.2}", v)}</span>
                                        </div>
                                    }).collect_view()}
                                </div>
                            }.into_any(),
