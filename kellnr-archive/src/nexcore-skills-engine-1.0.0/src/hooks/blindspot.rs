...
//!
//! Generates context-aware review prompts after file modifications
//! or subagent task completions to encourage thorough self-review.
...
impl BlindspotCheck {
    /// Generate a blindspot check for a file modification.
    ///
    /// Returns context-appropriate review items based on file type.
    ...
    /// Generate a blindspot check for a subagent task completion.
    ///
    /// Returns context-appropriate review items based on subagent type
...
mod tests {
    ...
    #[test]
    fn test_subagent_check_implementation() {
        let check = BlindspotCheck::for_subagent("general", "implement user login feature");

        assert_eq!(check.check_type, BlindspotType::Code);
        assert!(check.message.contains("Implementation"));
    }
