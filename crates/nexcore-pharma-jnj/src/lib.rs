...
//! Johnson & Johnson (Janssen Pharmaceuticals) pharmaceutical company model.
//!
//! Provides static catalog data for J&J's marketed products and pipeline
...
/// Johnson & Johnson company analysis handle.
pub struct JohnsonAndJohnson {
    data: Company,
}

impl JohnsonAndJohnson {
    /// Load the J&J company model.
    pub fn load() -> Self {
        Self {
...
impl CompanyAnalysis for JohnsonAndJohnson {
...
mod tests {
    ...
    #[test]
    fn company_loads() {
        let co = JohnsonAndJohnson::load();
        assert_eq!(co.company().ticker.as_deref(), Some("JNJ"));
        assert!(co.company().name.contains("Johnson"));
    }

    #[test]
    fn product_count_matches() {
        assert_eq!(JohnsonAndJohnson::load().company().products.len(), 8);
    }

    #[test]
    fn carvykti_has_boxed_warning() {
        // CAR-T therapies carry boxed warnings
        assert!(
            !JohnsonAndJohnson::load()
    ...
    #[test]
    fn daratumumab_present() {
        let found = JohnsonAndJohnson::load()
            .company()
            .products
            .iter()
            .any(|p| p.generic_name == "daratumumab");
        assert!(found, "Darzalex/daratumumab must be in catalog");
    }
}
