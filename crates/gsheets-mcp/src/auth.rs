//! Google OAuth2 authentication supporting both service accounts and user credentials.
//!
//! Credential search order:
//! 1. `GOOGLE_APPLICATION_CREDENTIALS` env var (service account JSON)
//! 2. `~/.config/gsheets-mcp/service-account.json` (service account fallback)
...
impl AuthManager {
    ...
    /// 2. `~/.config/gsheets-mcp/service-account.json`
    ...
    pub async fn new() -> Result<Self, AuthError> {
        ...
        match &source {
            ...
            CredentialSource::AuthorizedUser(_) => {
                info!("Using authorized_user (gcloud ADC) credentials");
            }
        }
        Ok(Self {
    ...
    pub async fn get_token(&self) -> Result<String, AuthError> {
        ...
        debug!("Refreshing Google Sheets access token");
    ...
    /// Acquire a token using the appropriate flow for the credential type.
...
async fn load_credentials() -> Result<CredentialSource, AuthError> {
    ...
    // 2. gsheets-mcp service account
    let sa_path = gsheets_sa_fallback();
    if sa_path.exists() {
...
async fn load_credential_file(path: &PathBuf) -> Result<CredentialSource, AuthError> {
    ...
    // Peek at the "type" field to determine credential kind.
    ...
    match cred_type {
        ...
        "authorized_user" => {
            let creds: UserCredentials =
                serde_json::from_str(&content).map_err(|e| AuthError::KeyParse(e.to_string()))?;
            info!(path = %path.display(), "Loaded authorized_user credentials");
...
fn gsheets_sa_fallback() -> PathBuf {
...
mod tests {
    ...
    #[test]
    fn gsheets_sa_fallback_contains_gsheets_mcp() {
        let path = gsheets_sa_fallback();
        assert!(
            path.to_string_lossy().contains("gsheets-mcp"),
            "fallback path should contain gsheets-mcp"
        );
    }
    ...
    #[test]
    fn token_response_missing_expires() {
        let json = r#"{"access_token":"ya29.test"}"#;
        let resp: TokenResponse = serde_json::from_str(json).expect("deserialize token response");
        assert!(resp.expires_in.is_none());
    }

    #[test]
    fn user_credentials_deserialize() {
        let json = r#"{
        ...
        let creds: UserCredentials =
            serde_json::from_str(json).expect("deserialize user credentials");
        assert!(creds.client_id.contains("googleusercontent"));
        assert_eq!(creds.refresh_token, "1//refresh");
    }

    #[test]
    fn detect_service_account_type() {
        ...
        let cred_type = raw
            ...
            .and_then(|v| v.as_str())
            .unwrap_or("unknown");
        assert_eq!(cred_type, "service_account");
    }

    #[test]
    fn detect_authorized_user_type() {
        ...
        let cred_type = raw
            ...
            .and_then(|v| v.as_str())
            .unwrap_or("unknown");
        assert_eq!(cred_type, "authorized_user");
    }
}
