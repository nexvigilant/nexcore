...
//!
//! This module provides the configuration system for file organization hooks.
...
///
/// Loaded from `~/.claude/file-policies.yaml` or a custom path.
...
pub struct PolicyFile {
    ...
    /// Forbidden zones where files should not be created
    pub forbidden_zones: Option<ForbiddenZones>,
    /// Expected project structure definitions
...
impl PolicySettings {
    /// Get the enforcement mode (default: "advisory")
    pub fn mode(&self) -> &str {
...
/// - `*` prefix for suffix matching (e.g., `*.py`)
/// - `*` suffix for prefix matching (e.g., `test_*`)
...
mod tests {
    ...
    fn test_policy_defaults() {
        ...
        assert_eq!(policy.stale_action(), "report");
        assert_eq!(policy.default_staleness_days(), 30);
    }
}
