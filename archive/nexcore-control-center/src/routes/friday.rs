...
use crate::api::{FridayComponents, FridayExecutor, FridaySource, FridayStatus, ProcessStatus};
use leptos::prelude::*;
...
pub async fn get_friday_status() -> Result<FridayStatus, ServerFnError> {
    ...
    let nexcore_api = "http://localhost:3030";
    let response = client
    ...
    Ok(FridayStatus {
        ...
        components: FridayComponents {
            event_bus: api
                .get("components")
                .and_then(|c| c.get("event_bus"))
                .cloned()
                .unwrap_or(serde_json::json!({})),
            memory_layer: api
                .get("components")
                .and_then(|c| c.get("memory_layer"))
                .cloned()
                .unwrap_or(serde_json::json!({})),
            decision_engine: api
                .get("components")
                .and_then(|c| c.get("decision_engine"))
                .cloned()
                .unwrap_or(serde_json::json!({})),
        },
        sources,
...
pub fn FridayPanel() -> impl IntoView {
    let status = Resource::new(|| (), |()| get_friday_status());
