...
async fn messages(
    ...
    if stream_mode {
        // Anthropic SSE protocol: emit message_start → content_block_start →
        // content_block_delta (one chunk with full text) → content_block_stop →
        // message_delta (stop_reason) → message_stop. Single-chunk streaming is
        // sufficient for CLI clients — native token streaming is Phase 2.
        let model_id = req.model.clone();
        let id_clone = id.clone();
        let events = vec![
            Event::default().event("message_start").json_data(serde_json::json!({
                "type": "message_start",
                "message": {
                    "id": id_clone,
                    "type": "message",
            ...
            Event::default().event("message_delta").json_data(serde_json::json!({
                "type": "message_delta",
                "delta": {"stop_reason": "end_turn", "stop_sequence": null},
                "usage": {"output_tokens": n_tok}
            })).unwrap_or_else(|_| Event::default()),
            Event::default().event("message_stop").json_data(serde_json::json!({
                "type": "message_stop"
            })).unwrap_or_else(|_| Event::default()),
        ];
        let (tx, rx) = tokio::sync::mpsc::channel::<std::result::Result<Event, std::convert::Infallible>>(events.len());
        for ev in events {
