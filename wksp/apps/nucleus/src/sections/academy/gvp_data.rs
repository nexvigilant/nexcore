...
#[cfg(feature = "hydrate")]
pub fn load_guardian_writeback() -> Vec<GuardianWritebackEvidence> {
    ...
    let Ok(Some(raw)) = storage.get_item(GUARDIAN_EVIDENCE_STORAGE_KEY) else {
        return Vec::new();
    };
    serde_json::from_str::<Vec<GuardianWritebackEvidence>>(&raw).unwrap_or_default()
}
...
#[cfg(feature = "hydrate")]
pub fn save_guardian_writeback(rows: &[GuardianWritebackEvidence]) {
    let Some(window) = web_sys::window() else {
        return;
    };
    let Ok(Some(storage)) = window.local_storage() else {
    ...
    let _ = storage.set_item(GUARDIAN_EVIDENCE_STORAGE_KEY, &payload);
}

#[cfg(not(feature = "hydrate"))]
pub fn save_guardian_writeback(_: &[GuardianWritebackEvidence]) {}

#[cfg(feature = "hydrate")]
pub fn clear_guardian_writeback() {
    let Some(window) = web_sys::window() else {
        return;
    };
    let Ok(Some(storage)) = window.local_storage() else {
    ...
    let _ = storage.remove_item(GUARDIAN_EVIDENCE_STORAGE_KEY);
}
...
#[cfg(feature = "hydrate")]
pub fn load_gvp_assessment_passes() -> Vec<GvpAssessmentPass> {
    let Some(window) = web_sys::window() else {
        return Vec::new();
    };
    let Ok(Some(storage)) = window.local_storage() else {
    ...
    let Ok(Some(raw)) = storage.get_item(GVP_ASSESSMENT_STORAGE_KEY) else {
        return Vec::new();
    };
    serde_json::from_str::<Vec<GvpAssessmentPass>>(&raw).unwrap_or_default()
}
...
#[cfg(feature = "hydrate")]
pub fn save_gvp_assessment_passes(rows: &[GvpAssessmentPass]) {
    let Some(window) = web_sys::window() else {
        return;
    };
    let Ok(Some(storage)) = window.local_storage() else {
    ...
    let payload = serde_json::to_string(rows).unwrap_or_else(|_| "[]".to_string());
    let _ = storage.set_item(GVP_ASSESSMENT_STORAGE_KEY, &payload);
}

#[cfg(not(feature = "hydrate"))]
pub fn save_gvp_assessment_passes(_: &[GvpAssessmentPass]) {}

#[cfg(feature = "hydrate")]
pub fn clear_gvp_assessment_passes() {
    let Some(window) = web_sys::window() else {
        return;
    };
    let Ok(Some(storage)) = window.local_storage() else {
    ...
    let _ = storage.remove_item(GVP_ASSESSMENT_STORAGE_KEY);
}
