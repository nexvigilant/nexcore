...
//! E - EMIT       : State → Signal              (T1: MAPPING)
//! S - (EXTEND)   : Representation → NewDomain  (T1: MAPPING)
//! ```
//!
//! ## Tier Classification
//!
...
/// T2-P: Produce a signal from internal state or computation.
///
/// Grounded in T1 MAPPING: internal → signal.
///
/// # Sense ↔ Emit Symmetry
///
/// `Sense` converts environment → signal (observation).
/// `Emit` converts internal state → signal (publication).
///
/// Together they complete the signal I/O contract:
/// ```text
/// Environment --[Sense]--> Signal --[process]--> Signal --[Emit]--> Consumers
/// ```
///
/// # Cross-Domain Transfer
///
/// | Domain | Emit Instantiation |
/// |--------|-------------------|
/// | PV | Signal detection result → regulatory report |
/// | Physics | Energy state → radiation emission |
/// | Economics | Decision → market signal |
/// | Software | State change → event publication |
///
/// # Examples
///
/// ```ignore
/// impl Emit for SignalDetector {
///     type Input = DetectionResult;
///     type Signal = SignalReport;
///
///     fn emit(&self, result: Self::Input) -> Self::Signal {
///         SignalReport::from(result)
///     }
/// }
/// ```
pub trait Emit {
    /// The internal state or computation result to emit
    type Input;
    /// The signal produced for external consumption
    type Signal;

    /// Convert internal state to emitted signal.
    fn emit(&self, input: Self::Input) -> Self::Signal;
}
...
///
/// Combines all eight T2-P primitives into a coherent methodology.
///
...
/// ```
///
/// `Emit` completes the Sense↔Emit symmetry: signals come in via `Sense`,
/// results go out via `Emit`. Without `Emit`, the loop has an asymmetric
/// I/O contract.
pub trait Science:
    Sense + Classify + Infer + Experiment + Normalize + Codify + Emit + Extend
{
