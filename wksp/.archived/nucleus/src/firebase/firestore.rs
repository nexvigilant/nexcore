...
//! via the REST API. Used for server-side data fetching (SSR).
...
/// Firestore REST client (SSR only)
#[cfg(feature = "ssr")]
...
fn parse_firestore_document<T: DeserializeOwned>(doc: &serde_json::Value) -> Result<T, String> {
...
fn convert_firestore_value(val: &serde_json::Value) -> serde_json::Value {
