...
/// Default CLI path. Overridable via `VIGIL_CLI_BIN` env var so an
/// operator can swap in a scoped wrapper (e.g. `~/nvnc/vigil-slim/claude`)
/// that passes `--bare --strict-mcp-config` to stay under the 32MB
/// request limit when the harness has hundreds of skills/plugins loaded.
const DEFAULT_CLI_BIN: &str = "/usr/bin/claude";
...
struct AppState {
    ...
    /// CLI binary path. Resolved once at startup from VIGIL_CLI_BIN env (if
    /// set) or DEFAULT_CLI_BIN. Leaked to &'static str to match existing
    /// ownership patterns.
    cli_bin: &'static str,
    started_at: Instant,
...
/// Build a CLI child command with unbounded posture. Works with any CLI
/// that accepts `--print --model <m> --output-format stream-json --verbose`
/// (Claude Code and Gemini CLI both honor this shape).
///
/// `cli_bin` is resolved from `VIGIL_CLI_BIN` env at startup, falling back
/// to `DEFAULT_CLI_BIN`.
///
...
async fn health(State(state): State<AppState>) -> impl IntoResponse {
    ...
    Json(HealthResp {
        status: "ok",
        scope: SCOPE,
        cli_bin: state.cli_bin.to_string(),
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Resolve CLI binary: VIGIL_CLI_BIN overrides DEFAULT_CLI_BIN. Leak
    // to 'static so it fits the existing AppState field shape.
    let cli_bin: &'static str = match std::env::var("VIGIL_CLI_BIN") {
        Ok(s) if !s.is_empty() => Box::leak(s.into_boxed_str()),
        _ => DEFAULT_CLI_BIN,
    };
    info!(cli_bin, "CLI resolved");
