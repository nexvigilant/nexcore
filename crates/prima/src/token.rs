...
/// Primary file extension for Prima source files.
///
/// `.true` — Code that compiles is mathematically true (grounded to 1).
pub const FILE_EXTENSION: &str = "true";

/// Fallback ASCII file extension.
pub const FILE_EXTENSION_FALLBACK: &str = "prima";
...
pub enum TokenKind {
    ...
    KappaEq,
    /// `κ!=` Kappa not-equals: explicit comparison
    KappaNe,
    /// `κ<` Kappa less: explicit comparison
    KappaLt,
    /// `κ>` Kappa greater: explicit comparison
    KappaGt,
    /// `κ<=` Kappa less-or-equal: explicit comparison
    KappaLe,
    /// `κ>=` Kappa greater-or-equal: explicit comparison
    KappaGe,
...
impl TokenKind {
    ...
    pub const fn dominant_primitive(&self) -> LexPrimitiva {
        match self {
            ...
            Self::Fn | Self::Arrow | Self::Return | Self::Pipeline => LexPrimitiva::Causality,
            ...
            Self::Comma | Self::Semicolon => LexPrimitiva::Sequence,
...
impl fmt::Display for TokenKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::KappaNe => write!(f, "κ!="),
            Self::KappaLt => write!(f, "κ<"),
            Self::KappaGt => write!(f, "κ>"),
            Self::KappaLe => write!(f, "κ<="),
            Self::KappaGe => write!(f, "κ>="),
            Self::LParen => write!(f, "("),
            Self::RParen => write!(f, ")"),
            Self::LBrace => write!(f, "{{"),
            Self::RBrace => write!(f, "}}"),
