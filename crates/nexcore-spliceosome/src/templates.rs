...
mod tests {
    ...
    fn test_all_categories_have_templates() {
        ...
        for cat in TaskCategory::ALL {
            assert!(templates.contains_key(&cat), "Missing template for {cat:?}");
            assert!(!templates[&cat].is_empty(), "Empty template for {cat:?}");
        }
    }
    ...
    fn test_mutate_has_verify_phase() {
        ...
        let has_verify = mutate
            .iter()
            .any(|m| m.expected_tool_categories.contains(&TaskCategory::Verify));
        ...
        for (cat, markers) in &templates {
            for m in markers {
                assert!(
                    ...
                    "Grounding threshold out of range for {cat:?}/{}",
                    m.phase_id
                );
            }
        }
