//! GroundsTo trait and LexPrimitiva — delegating to `nexcore-lex-primitiva`.
//!
//! Re-exports the canonical Lex Primitiva types from `nexcore-lex-primitiva`
//! and provides `GroundsTo` implementations for all nexcore-dna types (46 impls).
...
/// DrugEventSignal: T2-C (→ Causality + κ Comparison + N Quantity + ν Frequency)
/// Dominant: → Causality (signal implies causal link)
impl GroundsTo for DrugEventSignal {
    fn primitive_composition() -> PrimitiveComposition {
        PrimitiveComposition::new(vec![
            LexPrimitiva::Causality,
            LexPrimitiva::Comparison,
...
/// DnaArray: T2-P (σ Sequence + N Quantity + ∂ Boundary)
...
/// DnaRecord: T2-P (μ Mapping + λ Location + σ Sequence)
...
/// DnaMap: T2-P (μ Mapping + κ Comparison + ∃ Existence)
...
mod tests {
    ...
    fn nucleotide_is_t1() {
        ...
        assert_eq!(Tier::classify(&comp).code(), "T1");
    ...
    fn codon_is_t2p() {
        ...
        assert_eq!(Tier::classify(&comp).code(), "T2-P");
    ...
    fn double_helix_is_t2c() {
        ...
        assert_eq!(Tier::classify(&comp).code(), "T2-C");
        assert_eq!(comp.dominant, Some(LexPrimitiva::Sequence));
    }
    ...
    fn codon_vm_is_t3() {
        ...
        assert_eq!(Tier::classify(&comp).code(), "T3");
    ...
    #[test]
    fn signal_is_t2c() {
        let comp = DrugEventSignal::primitive_composition();
        assert_eq!(Tier::classify(&comp).code(), "T2-C");
        assert_eq!(comp.dominant, Some(LexPrimitiva::Causality));
        assert!((comp.confidence - 0.80).abs() < f64::EPSILON);
    }
    ...
    fn dna_array_is_t2p() {
        ...
        assert_eq!(Tier::classify(&comp).code(), "T2-P"); // 3 primitives → T2-P
        assert_eq!(comp.dominant, Some(LexPrimitiva::Sequence));
        assert!((comp.confidence - 0.80).abs() < f64::EPSILON);
    }
    ...
    fn dna_frame_is_t2c() {
        ...
        assert_eq!(Tier::classify(&comp).code(), "T2-C"); // 5 primitives → T2-C
        assert_eq!(comp.dominant, Some(LexPrimitiva::Sequence));
        assert!((comp.confidence - 0.60).abs() < f64::EPSILON);
    }
    ...
    fn json_value_is_t3() {
        ...
        assert_eq!(Tier::classify(&comp).code(), "T3"); // 6 primitives → T3
        assert_eq!(comp.dominant, Some(LexPrimitiva::Mapping));
