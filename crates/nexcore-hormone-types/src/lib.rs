//! # NexVigilant Core — Hormone Types
//!
//! Shared type definitions for the endocrine system, extracted to Foundation
//! layer so that stem-foundation crates can depend on hormone types without
//! pulling in the full bio-molecular nexcore-hormones crate.
...
mod tests {
    ...
    fn save_and_load_from_temp_path() {
        let dir = std::env::temp_dir().join("nexcore_hormone_types_test");
