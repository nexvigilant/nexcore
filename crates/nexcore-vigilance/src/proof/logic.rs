//! Core Logic Types for Curry-Howard Proofs
//!
...
//!
//! This implementation follows intuitionistic (constructive) logic, meaning:
//! - Proofs must be constructive (provide witnesses)
//! - Law of excluded middle (P ∨ ¬P) is NOT assumed
//! - Double negation elimination (¬¬P → P) is NOT available
...
pub fn double_neg_intro<P>(p: P) -> impl FnOnce(Not<P>) -> Void {
    // SAFETY: Void is uninhabited - this is intentional Curry-Howard encoding
...
pub fn modus_tollens<P, Q>(
    ...
    // SAFETY: Void is uninhabited - intentional Curry-Howard encoding
...
mod tests {
    ...
    fn test_and_introduction() {
        let proof: And<i32, &str> = And::intro(42, "hello");
        assert_eq!(proof.left, 42);
        assert_eq!(proof.right, "hello");
    }

    #[test]
    fn test_and_elimination() {
        let proof = And::intro(1, 2);
        assert_eq!(proof.elim_left(), 1);

        let proof = And::intro(1, 2);
        assert_eq!(proof.elim_right(), 2);
    }

    #[test]
    fn test_and_commute() {
        let proof = And::intro(1, "a");
        let commuted = proof.commute();
        assert_eq!(commuted.left, "a");
        assert_eq!(commuted.right, 1);
    }

    #[test]
    fn test_or_introduction() {
        let left: Or<i32, &str> = Or::intro_left(42);
        assert!(matches!(left, Or::Left(42)));

        let right: Or<i32, &str> = Or::intro_right("hello");
        assert!(matches!(right, Or::Right("hello")));
    }

    #[test]
    fn test_or_elimination() {
        let left: Or<i32, i32> = Or::Left(1);
        let result = left.elim(|x| x * 2, |x| x * 3);
        assert_eq!(result, 2);

        let right: Or<i32, i32> = Or::Right(1);
        let result = right.elim(|x| x * 2, |x| x * 3);
        assert_eq!(result, 3);
    }
    ...
    fn test_exists() {
        // Prove: ∃n. n > 0
        let proof: Exists<u32, ()> = Exists::intro(42, ());
        assert_eq!(proof.witness, 42);
    }
    ...
    fn test_hypothetical_syllogism() {
        ...
        let composed = hypothetical_syllogism(add_one, double);
        assert_eq!(composed(5), 12); // (5 + 1) * 2 = 12
    }

    #[test]
    fn test_proof_marker() {
        struct IsPositive;
        let _proof: Proof<IsPositive> = Proof::qed();
    }

    #[test]
    fn test_trivial() {
        let _truth: Truth = trivial();
    }

    #[test]
    fn test_distribute_and_over_or() {
        let proof: And<i32, Or<&str, bool>> = And::intro(1, Or::Left("a"));
        let result = distribute_and_over_or(proof);
        assert!(matches!(
            result,
            Or::Left(And {
                left: 1,
