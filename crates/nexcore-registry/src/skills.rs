...
pub fn insert(conn: &Connection, row: &SkillRow) -> Result<()> {
    ...
    conn.execute(
        ...
        params![
            row.name, row.path, row.description, row.argument_hint,
            row.disable_model_invocation, row.user_invocable, row.allowed_tools,
            row.model, row.context, row.agent, row.hooks,
            row.line_count, row.has_agent, row.sub_skill_count, row.parent_skill,
            row.uses_arguments, row.uses_dynamic_context, row.uses_session_id,
            row.content_chars,
            row.smst_input, row.smst_output, row.smst_logic, row.smst_error_handling,
            row.smst_examples, row.smst_references, row.last_assessed_at, row.assessed_by,
            row.version, row.compliance, row.smst_v1, row.smst_v2,
            row.tags, row.chain_position, row.pipeline,
            scanned, updated,
        ],
    )?;
    Ok(())
...
pub fn list_all(conn: &Connection) -> Result<Vec<SkillRow>> {
    let mut stmt = conn.prepare(
        &format!("SELECT {COLUMNS} FROM active_skills ORDER BY name"),
    )?;
