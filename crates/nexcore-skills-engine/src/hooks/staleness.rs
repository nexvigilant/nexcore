...
//!
//! Detects files that haven't been modified within configured thresholds,
//! helping identify potentially orphaned or outdated files.
...
impl StalenessResult {
    /// Create a new staleness result with defaults
...
///
/// The staleness threshold is determined by:
/// 1. Category-specific rules from placement rules
/// 2. Path-specific rules from staleness config
/// 3. Default threshold (30 days)
...
/// Summary of staleness analysis for a directory scan.
...
impl StalenessSummary {
    /// Add a staleness result to the summary
...
mod tests {
    ...
    #[test]
    fn test_format_staleness_result() {
        let mut result = StalenessResult::new("/test/old.tmp", "temp_files");
        result.is_stale = true;
        result.age_days = 45;
        result.threshold_days = 30;

        let formatted = format_staleness_result(&result);
        assert!(formatted.contains("[Stale]"));
        assert!(formatted.contains("45 days old"));
    }
}
