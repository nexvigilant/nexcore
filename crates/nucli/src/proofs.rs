...
mod tests {
    ...
    fn p2_decode_encode_roundtrip_exhaustive() {
        ...
        for &n0 in &nucs {
            for &n1 in &nucs {
                for &n2 in &nucs {
                    for &n3 in &nucs {
                        let tetrad: String = [n0, n1, n2, n3].iter().collect();
                        let decoded = codec::decode(&tetrad);
                        assert!(decoded.is_ok(), "decode failed for tetrad {tetrad:?}");
    ...
    fn p3_encode_injectivity_exhaustive() {
        ...
        for b in 0u16..=255 {
            ...
            if let Some(prev) = seen.insert(strand.clone(), byte) {
                panic!("collision: bytes {prev:#04x} and {byte:#04x} both encode to {strand:?}");
            }
        }
    ...
    fn p9_complement_involution_multibyte() {
        ...
        for data in test_bytes {
            ...
            assert_eq!(strand, rc2, "multi-byte involution failed for {data:?}");
        }
    }
