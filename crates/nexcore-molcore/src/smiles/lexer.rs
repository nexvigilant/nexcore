        ...
        match ch {
            ...
            'B' => {
                if pos + 1 < chars.len() && chars[pos + 1] == 'r' {
                    tokens.push(SmilesToken::OrganicAtom {
                        symbol: "Br".into(),
                        aromatic: false,
                    });
                    pos += 2;
            ...
            // Bonds
            '-' => {
                tokens.push(SmilesToken::Bond(BondToken::Single));
                pos += 1;
            }
            '=' => {
                tokens.push(SmilesToken::Bond(BondToken::Double));
                pos += 1;
            }
            '#' => {
                tokens.push(SmilesToken::Bond(BondToken::Triple));
                pos += 1;
            }
            ':' => {
                tokens.push(SmilesToken::Bond(BondToken::Aromatic));
                pos += 1;
            }
            '/' => {
                tokens.push(SmilesToken::Bond(BondToken::Up));
                pos += 1;
            }
            '\\' => {
                tokens.push(SmilesToken::Bond(BondToken::Down));
                pos += 1;
            }
            // Ring closures
            '%' => {
            ...
            // Wildcard
            '*' => {
                tokens.push(SmilesToken::Wildcard);
                pos += 1;
            }
            // Whitespace — skip
            ' ' | '\t' | '\n' | '\r' => {
