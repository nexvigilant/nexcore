...
fn process_cube(
    ...
    for (ei, &[c0, c1]) in CUBE_EDGES.iter().enumerate() {
        if EDGE_TABLE[cube_index] & (1 << ei) != 0 {
            edge_verts[ei] = interpolate_edge(
                corner_pos[c0],
                corner_pos[c1],
                corner_vals[c0],
                corner_vals[c1],
...
const EDGE_TABLE: [u16; 256] = [
    ...
    0xD30, 0xC39, 0xF33, 0xE3A, 0x936, 0xB35, 0xB3F, 0xA36, 0x53C, 0x435, 0x73F, 0x636, 0x13A,
...
mod tests {
    ...
    fn vdw_surface_water_non_empty() {
        ...
        assert!(
            ...
            "VdW surface for water must produce vertices"
        );
        assert!(
            !mesh.indices.is_empty(),
            "VdW surface for water must produce triangles"
        );
        assert!(
            !mesh.colors.is_empty(),
            "VdW surface for water must produce colors"
        );
    }
    ...
    fn vertex_and_triangle_count_helpers() {
        ...
        assert_eq!(
            mesh.indices.len(),
            tc * 3,
            "indices.len() == triangle_count * 3"
        );
        assert_eq!(
            mesh.vertices.len(),
            vc * 3,
            "vertices.len() == vertex_count * 3"
        );
    }
    ...
    fn ses_surface_non_empty() {
        ...
        assert!(
            !mesh.vertices.is_empty(),
            "SES surface for water must produce vertices"
        );
    }
    ...
    fn single_atom_carbon_has_surface() {
        ...
        assert!(
            !mesh.vertices.is_empty(),
            "Single carbon atom must yield a surface"
        );
    }
